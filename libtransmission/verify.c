    ...
    while (!*stopFlag && (pieceIndex < tor->info.pieceCount))
    {
        ...
        if (filePos == 0 && fd == TR_BAD_SYS_FILE && fileIndex != prevFileIndex)
        {
            ...
            fd = filename == NULL ? TR_BAD_SYS_FILE : tr_sys_file_open (filename,
                                                                        TR_SYS_FILE_READ, 0, NULL);
            file_length = 0;
            if (fd != TR_BAD_SYS_FILE) {
                tr_sys_path_info file_info;
                bool get_info_success = tr_sys_file_get_info(fd, &file_info, NULL);
                if (get_info_success) {
        ...
        if (fd != TR_BAD_SYS_FILE)
        {
            uint64_t left_in_actual_file;
            if (file_length < filePos)
        ...
        if (leftInPiece == 0)
        {
            time_t now;
            bool hasPiece;
            hasPiece = tr_torPieceCountBytes(tor, pieceIndex) == piece_len_on_file;
...
static void
verifyNoHashCheckThreadFunc (void * unused UNUSED)
{
    for (;;)
    {
        int changed = 0;
...
void
tr_verifyAddNoHashCheck (tr_torrent           * tor,
              tr_verify_done_func    callback_func,
              void                 * callback_data)
{
    struct verify_node * node;
